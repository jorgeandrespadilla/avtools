...
from pytube import YouTube, StreamQuery, exceptions
...
class DownloadCommand:
    ...
    5. [CLEANUP] Remove the temporary files (video and audio).
    """
    ...
    def execute(self) -> None:
        with Progress(
            ...
            download_task = progress.add_task(
                "[yellow]Downloading video...", total=1, completed=0, visible=False
            )
            ...
            def on_complete_callback(stream, _file_handle):
                progress.update(
                    download_task,
                    completed=stream.filesize,
                    description="[green]Download completed",
                )

            try:
                yt = YouTube(
                    self.params.input_url,
            ...
            video_stream = streams.filter(res=self.params.target_resolution).first()
            ...
            video_stream.download(
                str(self.params.output_file_path.directory_path),
                filename=self.params.output_file_path.full_name,
